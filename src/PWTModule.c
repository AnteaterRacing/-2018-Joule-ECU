...
void init_PWTModule(void)  {
  ...
  PWT_R1 = 0x0000000; 	/* Initialize PWT for measuring falling edges */
  	  	  	  	  	  	/* We Want:									  */
                        ...
                        /* PRDYIE (PWT pulse width data ready interrupt ena)= 1 (interrupt when pulse is ready to read) */
                        /* POVIE (PWT Counter overflow interrupt ena) = 1 (interrupt when counter overflows [car is moving less than 3 miles per hour]) */
...
void PWT_IRQHandler(void) {
	//transmit_string("interrupt");

	/* The interrupt handler can handle two cases. This interrupt handler can first put the retrieved value into the correct spot in the buffer then call calculateWheelSpeed
	 * to calculate the actual speed. It also detects if the counter overflows, which happens either if the magnet is not detected for a long period of time or if the magnet sits
	 * still in the sensor's detection range for a long period of time. Finally, the interrupt handler will switch detection of the pulse width to the other sensor that is not
...
//		transmit_string("\n\r");
		...
		if (0 == (PWT_R1 & PWT_R1_PINSEL_MASK) >> PWT_R1_PINSEL_SHIFT){ // Check if the PWT is connect to the left or right wheel sensor
			//transmit_string("putting in buffer");
